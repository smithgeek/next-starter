							...
							<div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg border border-border bg-card">
								<div className="rounded-t mb-0 px-6 py-6">
									<div className="text-center mb-3 flex flex-col justify-center">
										{view === "login" && (
											<h6 className="text-sm font-bold">Sign in {providers.length > 0 ? "with" : ""}</h6>
										)}
									</div>
									{view === "login" && (
										<>
											<div className="btn-wrapper text-center flex justify-around">
												...
												{providers.includes("google") && (
													<Button
														...
														disabled={signIn.isPending}
													>
														<img alt="Sign in with google" className="w-5 mr-1" src="/assets/img/google.svg" />
														Google
													</Button>
												)}
											</div>
											{providers.length > 0 && <hr className="mt-6 border-b-1 border-blueGray-300" />}
										</>
									)}
									{view === "checkEmail" && (
										<div className="w-full flex justify-center mb-2">
											<Label>Check your email for a sign in link.</Label>
										</div>
									)}
								</div>
								{view !== "checkEmail" && (
									<div className="flex-auto px-4 lg:px-10 py-10 pt-0">
										{providers.length > 0 && view === "login" && (
										...
										<Form {...form}>
											...
											>
												...
												<Button className="w-full" type="submit" busy={signIn.isPending}>
													Sign In
												</Button>
											</form>
										</Form>
